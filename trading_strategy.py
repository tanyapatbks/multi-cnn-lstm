...
class ForexPortfolioManager:
    ...
    def calculate_position_size(self, confidence_level='moderate', currency_pair='EURUSD'):
        ...
        # Calculate maximum loss for this position
        # Risk = lot_size * stop_loss_pips * pip_value
        max_loss = lot_size * self.config.STOP_LOSS_PIPS * pip_value
        
        # Check if risk exceeds allowed percentage
        max_allowed_loss = self.capital * self.config.RISK_PER_TRADE_PCT
        
        # Adjust lot size if risk is too high
        ...
        position_value = units * 1.0  # Simplified (assuming price ~1.0)
    ...
    def calculate_sharpe_ratio(self):
        ...
        # Assuming 252 trading days * 24 hours
    ...
    def get_performance_metrics(self):
        ...
        return {
            ...
            'max_capital': np.max(capital_array),
            'min_capital': np.min(capital_array),
...
class FixedHoldingTradingStrategy:
    ...
    def apply_strategy(self, predictions, prices, timestamps, threshold_type='moderate', 
        ...
        """
        Apply fixed holding period trading strategy with confidence-based lot sizing
        
        The strategy has 3 zones:
        - Buy zone: prediction >= buy_threshold
        - Sell zone: prediction <= sell_threshold  
        - Hold zone: between thresholds (no action)
        """
        if verbose:
        ...
        # Get thresholds for the strategy
        ...
        for i, (pred, price, timestamp) in enumerate(zip(predictions, prices, timestamps)):
            ...
            if current_position != 0:
                ...
                if current_position == 1:  # Long
                    current_pips = (price - entry_price) * pip_multiplier
                else:  # Short
                    current_pips = (entry_price - price) * pip_multiplier
                
                should_close = False
                close_reason = ''
                
                # Stop loss check (in pips)
                if current_pips <= -self.config.STOP_LOSS_PIPS:
                    should_close = True
                    close_reason = 'stop_loss'
                
                # Take profit check (in pips)
                ...
                # Time-based rules
                    ...
                    elif current_pips > 0:  # Take any profit after min holding
                        should_close = True
                        close_reason = 'profit_taking'
                
                if should_close:
                    # Execute trade with forex position sizing
                    trade_record = portfolio_manager.execute_forex_trade(
                        ...
                        timestamp,
                        currency_pair,
                        threshold_type  # Use threshold type as confidence level
                    )
                    ...
                    trade = {
                        ...
                        'entry_price': entry_price,
                        'exit_price': price,
                        'pips_gained': trade_record['pips_gained'],
                        'lot_size': trade_record['lot_size'],
                        'pnl': trade_record['pnl'],
                        'pnl_pct': trade_record['pnl_pct'],
                        'holding_hours': hours_held,
                        'reason': close_reason,
                        'confidence_level': threshold_type
                    }
                    trades.append(trade)
            ...
            # THREE ZONES: Buy, Sell, Hold
            if current_position == 0:
                if pred >= thresholds['buy']:
                    # BUY ZONE - High confidence to go up
                    signals[i] = 1
                    current_position = 1
                    entry_price = price
                    ...
                    # SELL ZONE - High confidence to go down
                    signals[i] = -1
                    current_position = -1
                    entry_price = price
                    entry_time = timestamp
                    entry_idx = i
                # else: HOLD ZONE - No action (between thresholds)
            
            positions[i] = current_position
        
        # Handle remaining position at the end
        if current_position != 0:
            final_timestamp = get_last_timestamp(timestamps)
        ...
        if verbose:
            ...
            print(f"      Total pips: {portfolio_metrics['total_pips']:.1f}")
            print(f"      Final capital: ${portfolio_metrics['final_capital']:,.2f}")
            print(f"      Total return: {portfolio_metrics['total_return_pct']:.2f}%")
            print(f"      Win rate: {portfolio_metrics['win_rate']:.4f}")
        ...
        return {
            ...
            'lot_size': self.config.LOT_SIZES[threshold_type],
            'portfolio_manager': portfolio_manager,
            'capital_history': portfolio_manager.capital_history,
            'zone_stats': {
    ...
    def compare_strategies(self, predictions, prices, timestamps, currency_pair='EURUSD', verbose=True):
        ...
        for strategy_type in ['conservative', 'moderate', 'aggressive']:
            # Create new portfolio for each strategy
            portfolio_manager = ForexPortfolioManager(self.config)
            
            result = self.apply_strategy(
                predictions, prices, timestamps, 
                strategy_type, portfolio_manager, 
                currency_pair, verbose=False
            )
    ...
    def apply_multi_model_to_all_pairs(self, model_builder, processed_data, data_splits, 
        ...
        """
        Apply multi-currency model to each currency pair separately
        This is the correct way to evaluate multi-model performance
        """
        if verbose:
        ...
        for currency_pair in self.config.CURRENCY_PAIRS:
            ...
            # Multi-model makes predictions on multi-currency input
            predictions = model_builder.predict(X_eval)
            
            # Get price data for this specific currency pair
            ...
            # Compare all three strategies for this pair
            pair_results = self.compare_strategies(
                predictions, eval_prices, eval_timestamps, 
                currency_pair, verbose=verbose
            )
...
class TechnicalIndicatorStrategies:
    ...
    def rsi_strategy(self, prices, timestamps, technical_indicators, 
        ...
        """RSI-based trading strategy with forex position sizing"""
        if verbose:
        ...
        # Use moderate lot size for technical indicators
        confidence_level = 'moderate'
        
        for i, (price, timestamp, rsi) in enumerate(zip(prices, timestamps, rsi_values)):
            # Exit logic
            if current_position == 1 and rsi >= self.config.RSI_OVERBOUGHT:
                # Exit long position
                trade_record = portfolio_manager.execute_forex_trade(
                    'long', entry_price, price, timestamp, 
            ...
            elif current_position == -1 and rsi <= self.config.RSI_OVERSOLD:
                # Exit short position
                trade_record = portfolio_manager.execute_forex_trade(
                    'short', entry_price, price, timestamp,
                    currency_pair, confidence_level
                )
                trades.append({
                    'type': 'short',
        ...
        if current_position != 0:
            ...
            trade_record = portfolio_manager.execute_forex_trade(
                'long' if current_position == 1 else 'short',
                entry_price, final_price, final_timestamp,
                currency_pair, confidence_level
            )
            trades.append({
                'type': 'long' if current_position == 1 else 'short',
        ...
        return {
            'strategy_name': 'MACD-based Trading',
            'trades': trades,
            'positions': positions,
            'performance': portfolio_metrics,
            'portfolio_manager': portfolio_manager,
...
class SimpleBaselineStrategies:
    ...
    def buy_and_hold(self, prices, timestamps, currency_pair='EURUSD', portfolio_manager=None):
        ...
        # Use conservative lot size for buy and hold
        confidence_level = 'conservative'
        
        # Handle both Series and DatetimeIndex for timestamps
    ...
    def random_strategy(self, prices, timestamps, currency_pair='EURUSD', 
        ...
        # Use moderate lot size for random
        confidence_level = 'moderate'
        ...
        for _ in range(num_trades):
            ...
            # Handle both Series and DatetimeIndex for timestamps
            entry_time = get_timestamp_value(timestamps, entry_idx)
            exit_time = get_timestamp_value(timestamps, exit_idx)
            
            # Random position (long or short)
        ...
        _price, final_timestamp, currency_pair, confidence_level
        ...
        return {
            'strategy_name': 'RSI-based Trading',
            'trades': trades,
            'positions': positions,
            'performance': portfolio_metrics,
            'portfolio_manager': portfolio_manager,
    ...
    def macd_strategy(self, prices, timestamps, technical_indicators, 
                     currency_pair='EURUSD', portfolio_manager=None, verbose=True):
        """MACD-based trading strategy with forex position sizing"""
        if verbose:
        ...
        # Skip first few periods to allow MACD to stabilize
        start_idx = max(self.config.MACD_SLOW, 50)
        
        for i in range(start_idx, len(prices)):
            price = prices.iloc[i]
            timestamp = get_timestamp_value(timestamps, i)
            ...
            if i > 0:
                ...
                # Exit logic
                if current_position == 1 and bearish_cross:
                    # Exit long position
                    trade_record = portfolio_manager.execute_forex_trade(
                        'long', entry_price, price, timestamp,
                        currency_pair, confidence_level
                ...
                elif current_position == -1 and bullish_cross:
                    # Exit short position
                    trade_record = portfolio_manager.execute_forex_trade(
                        'short', entry_price, price, timestamp,
                        currency_pair, confidence_level
        ...
        if current_position != 0:
            ...
            trade_record = portfolio_manager.execute_forex_trade(
                'long' if current_position == 1 
                    else 'short', entry_price, final )